...
//#define SEALEVELPRESSURE_HPA (1013.25) // Usefull if determining current height --> not really interestng in this static case...
...
const MessageDef MessageTable[] PROGMEM = {
  {SCREEN_TEMPERATURE,"Temperatuur"}, //"Temperature"
  {SCREEN_HUMIDITY,"Luchtvochtigheid"}, // "Humidity"
  {SCREEN_FORECAST,"Barometer"},
  {SCREEN_STABLE,"Stabiel"},
  {SCREEN_SUNNY,"Zonnig"}, //"SUNNY"
  {SCREEN_CLOUDY,"Bewolkt"},
  {SCREEN_UNSTABLE,"Instabiel"},
  {SCREEN_THUNDERSTORM,"Onweer"}, //"Thunderstorm"
  {SCREEN_UNKNOWN,"Onbekend"},
  };
...
//IM BEING VERY NAUGHT, AND REUSE THIS VARIABLE TO SAVE SPACE!
#define RADIO_DELAY 100                             // milliseconds betweeen radio signals during the presentation phase. DO NOT CHANGE
...
//boolean metric = true;                              // Should the device show metric or Fahrenheit? --> Just show fucking metric. No choises here. Removed Fahrenheit to save some space


// Here comes some stuff that I hope places it as global varaible, so it doesnt take up space
float lastPressureSamplesAverage;
float change;
byte string_length;
char char_byte;
...
// VOIDS I MOVED FROM END OF FILE HERE - START
...
int touchToY(int y)
{
  return int constrain(((y - RADIO_DELAY) / 2.8), 0, TOUCHSCREEN_HEIGHT); // RADIO_DELAY=100, to save space
}
...
// VOIDS I MOVED FROM END OF FILE HERE - END
...
//#endif
  ...
  if(isTransportReady()){                           // If we are connected to the MySensors network.
    ...
    connectedToNetwork = true;
    //metric = getControllerConfig().isMetric;        // Ask if Celcius or Fahrenheit is prefered. ASUME METRIC!
  }else{
  ...
  status = bme.begin();  
  if (!status) {
      //Serial.println("Could not find sensor, check stuff!");
...
//    Serial.print("counter:");Serial.print(intervalCounter);
...
//        if(temperature != -127.00 && temperature != 85.00 && temperature != previous_temperature) { // Avoids working with measurement errors. FUCK measurements errors!
//          previous_temperature = temperature;
...
//        Serial.print(F("Sending humidity ")); Serial.println(current_humidity);
    ...
    case 2:
      ...
      pressure = round(bme.readPressure() / RADIO_DELAY); // RADIO_DELAY=100 to save space
//      Serial.print(F("Sending pressure ")); Serial.println(pressure);
      send(pressure_message.set((pressure),1));

      forecast = sample(pressure/RADIO_DELAY); // RADIO_DELAY=100 to save space
